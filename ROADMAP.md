...
## ✅ Phase 3.5: 一覧・閲覧 UI ← **2026-04-14 完了 / 2026-04-17 パフォーマンス強化**
...
**Phase 3.5 ZK復号パフォーマンス強化 ← 2026-04-17 完了**
- [x] 3.5.7 インクリメンタルバッチ復号 — `DECRYPT_BATCH_SIZE=15` でバッチ内並列化
  - 逐次200往復 → 14バッチ（`Promise.allSettled`）に削減
  - バッチ間に `yieldToMain()` を挿入しメインスレッドのJankを防止
  - `scheduler.yield()` → `requestIdleCallback` → `setTimeout(0)` の3段フォールバック
- [x] 3.5.8 プログレッシブレンダリング — 第1バッチ完了後に即レンダリング開始
  - 50枚: 全件表示まで ~33ms (体感: 即時)
  - 200枚: 第1カード ~10ms、全件 ~200ms (Jank なし)
- [x] 3.5.9 Web Worker採用を意図的に見送り（設計判断）
  - CryptoKey の Worker 転送は security boundary を曖昧にする
  - I/Oヘビー（非CPU）な処理にWorkerは過剰。yieldで十分対応可能
